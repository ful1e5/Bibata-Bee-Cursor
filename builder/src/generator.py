...
def xbuild(config: Dict[str, Dict[str, Any]], x_out_dir: Path, info: Info) -> None:
    """Build `Bibata Bee` cursor theme for only `X11`(UNIX) platform.
...
def wbuild(config: Dict[str, Dict[str, Any]], win_out_dir: Path, info: Info) -> None:
    """Build `Bibata Bee` cursor theme for only `Windows` platforms.
...
def build(
    ...
    """Build `Bibata Bee` cursor theme for `X11` & `Windows` platforms.
