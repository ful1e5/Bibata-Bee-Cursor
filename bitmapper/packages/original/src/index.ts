...
import { BitmapsGenerator, SVGHandler } from "bibata-bee-core";

const root = path.resolve(__dirname, "../../../../");
const svgDir = path.resolve(root, "svg", "original");

const themeName = "Bibata-Bee-Original";
...
const main = async () => {
  ...
  for (let { key, content } of svg.getAnimated()) {
    ...
    if (trueAnimated.includes(key)) {
      await png.generateAnimated(browser, content, key, { playbackRate: 0.6 });
    } else {
